...
let checkInvalid = () => {
    ...
    for (let key in storeInformation) {
        ...
        } else if (storeInformation["passwordIN"].length <= 6) {
            for (let input of allInput) {
...
backPlaylistBtn.addEventListener("click", (evt) => {
 ...
 async function getFolders(){
    let promises = await fetch("http://127.0.0.1:5500/musicPlayer/folder/")
    ...
    as.forEach(async(a)=> {
        if(a.href.includes("/folder/")){
          ...
          let response = await fetch(`/musicPlayer/folder/${song}/info.json`);
          let data = await response.json()
          let folderDiv = document.createElement("div");
          folderDiv.classList.add("folder");
          folderDiv.innerHTML=`<a href="${a.href}"></a><img src="/musicPlayer/folder/${song}/folderImg.jpg" alt="coverimg">
